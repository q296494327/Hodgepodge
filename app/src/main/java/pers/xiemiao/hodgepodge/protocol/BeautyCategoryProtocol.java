...
import pers.xiemiao.hodgepodge.bean.BeautyCategoryBean;
 ...
 * Desc: 美女分类网络协议（过审）
 */
public class BeautyCategoryProtocol {
     ...
     * @param type 数据加载类型
     */
    public BeautyCategoryBean loadData(String type, int page) throws Exception {
        if (!isRefresh) {
            //1先去获取本地缓存
            BeautyCategoryBean localBean = getDataFromLocal(type, page);
        ...
        isRefresh = false;
        //2当本地缓存为空时，就去返回网络数据
        BeautyCategoryBean netBean = getDataFromNet(type, page);
        if (netBean != null) {
            return netBean;
        }
        return null;
     ...
     * @param page   第几页
     */
    private BeautyCategoryBean getDataFromLocal(String params, int page) {
        //1指定存储路径以及存储名字
     ...
     * @throws IOException
     */
    private BeautyCategoryBean getDataFromNet(String params, int page) throws IOException {
        String url = Constants.URLS.BEAUTYCATEGORYURL + "&id=" + params + "&page=" + page;
     ...
     */
    protected BeautyCategoryBean parseJson(String jsonString) {
        return new Gson().fromJson(jsonString, BeautyCategoryBean.class);
    }
