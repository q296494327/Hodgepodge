 ...
 * Desc: 鬼故事分类网络协议
 */
public class GhostCategoryProtocol {
     ...
     * @param type 数据加载类型
     */
    public GhostCategoryBean loadData(String type, int page) throws Exception {
        if (!isRefresh) {
            //1先去获取本地缓存
            GhostCategoryBean localBean = getDataFromLocal(type, page);
        ...
        isRefresh = false;
        //2当本地缓存为空时，就去返回网络数据
        GhostCategoryBean netBean = getDataFromNet(type, page);
        if (netBean != null) {
            return netBean;
        }
        return null;
     ...
     * @param page   第几页
     */
    private GhostCategoryBean getDataFromLocal(String params, int page) {
        //1指定存储路径以及存储名字
     ...
     * @throws IOException
     */
    private GhostCategoryBean getDataFromNet(String params, int page) throws IOException {
        String url = Constants.URLS.GHOSTCATEGORYURL + "&type=" + params + "&page=" + page;
     ...
     */
    protected GhostCategoryBean parseJson(String jsonString) {
        return new Gson().fromJson(jsonString, GhostCategoryBean.class);
    }
