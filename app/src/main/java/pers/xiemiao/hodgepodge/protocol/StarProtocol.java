...
public class StarProtocol {
     ...
     * @param params 数据加载类型
     */
    public StarBean loadData(String params) throws Exception {
        if (!isRefresh) {
            //1先去获取本地缓存
            StarBean localBean = getDataFromLocal(params);
        ...
        isRefresh = false;
        //2当本地缓存为空时，就去返回网络数据
        StarBean netBean = getDataFromNet(params);
        if (netBean != null) {
            return netBean;
        }
        return null;
     ...
     * @param params 数据加载类型
     */
    private StarBean getDataFromLocal(String params) {
        //1指定存储路径以及存储名字
     ...
     * @throws IOException
     */
    private StarBean getDataFromNet(String params) throws IOException {
        String url = Constants.URLS.STARURL + "&star=" + params;
     ...
     */
    protected StarBean parseJson(String jsonString) {
        return new Gson().fromJson(jsonString, StarBean.class);
    }
