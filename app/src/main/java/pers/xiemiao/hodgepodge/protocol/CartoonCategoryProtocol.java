 ...
 * Desc: 卡通分类网络协议
 */
public class CartoonCategoryProtocol {
     ...
     * @param type 数据加载类型
     */
    public CartoonCategoryBean loadData(String type, int page) throws Exception {
        if (!isRefresh) {
            //1先去获取本地缓存
            CartoonCategoryBean localBean = getDataFromLocal(type, page);
        ...
        isRefresh = false;
        //2当本地缓存为空时，就去返回网络数据
        CartoonCategoryBean netBean = getDataFromNet(type, page);
        if (netBean != null) {
            return netBean;
        }
        return null;
     ...
     * @param page   第几页
     */
    private CartoonCategoryBean getDataFromLocal(String params, int page) {
        //1指定存储路径以及存储名字
     ...
     * @throws IOException
     */
    private CartoonCategoryBean getDataFromNet(String params, int page) throws IOException {
        String url = Constants.URLS.CARTOONCATEGORYURL + "&type=/category/" + params + "&page=" +
                page;
     ...
     */
    protected CartoonCategoryBean parseJson(String jsonString) {
        return new Gson().fromJson(jsonString, CartoonCategoryBean.class);
    }
