 ...
 * Desc: 美女详情网络协议（过审）
 */
public class BeautyDetailProtocol {
     ...
     * @param id 数据加载类型
     */
    public BeautyDetailBean loadData(int id) throws Exception {
        if (!isRefresh) {
            //1先去获取本地缓存
            BeautyDetailBean localBean = getDataFromLocal(id);
        ...
        isRefresh = false;
        //2当本地缓存为空时，就去返回网络数据
        BeautyDetailBean netBean = getDataFromNet(id);
        if (netBean != null) {
            return netBean;
        }
        return null;
     ...
     * @param id 数据加载类型
     */
    private BeautyDetailBean getDataFromLocal(int id) {
        //1指定存储路径以及存储名字
     ...
     * @throws IOException
     */
    private BeautyDetailBean getDataFromNet(int id) throws IOException {
        String url= Constants.URLS.BEAUTYDETAILURL + "&id=" + id;
        Response response = OkHttpUtils.get().url(url).build().execute();
        String jsonString = response.body().string();
        if (!StringUtils.isEmpty(jsonString)) {
            LogUtils.sf("写缓存");
            //将json进行缓存
            FileUtils.writeFile(jsonString, getCacheFile(id).getAbsolutePath(), false);
            //解析json,由子类去实现
    ...
    private File getCacheFile(int id) {
        ...
        String name = Constants.URLS.BEAUTYDETAILURL.replace("/", "").replace("?", "").replace
                (":", "") + "." + id;
     ...
     */
    protected BeautyDetailBean parseJson(String jsonString) {
        return new Gson().fromJson(jsonString, BeautyDetailBean.class);
    }
