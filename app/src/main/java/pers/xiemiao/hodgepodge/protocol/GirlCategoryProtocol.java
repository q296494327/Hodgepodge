 ...
 * Desc: 美女分类网络协议
 */
public class GirlCategoryProtocol {
     ...
     * @param params 数据加载类型
     */
    public GirlCategoryBean loadData(String params) throws Exception {
        if (!isRefresh) {
            //1先去获取本地缓存
            GirlCategoryBean localBean = getDataFromLocal(params);
        ...
        isRefresh = false;
        //2当本地缓存为空时，就去返回网络数据
        GirlCategoryBean netBean = getDataFromNet(params);
        if (netBean != null) {
            return netBean;
        }
        return null;
     ...
     * @param params 数据加载类型
     */
    private GirlCategoryBean getDataFromLocal(String params) {
        //1指定存储路径以及存储名字
     ...
     * @throws IOException
     */
    private GirlCategoryBean getDataFromNet(String params) throws IOException {
        String url;
     ...
     */
    protected GirlCategoryBean parseJson(String jsonString) {
        return new Gson().fromJson(jsonString, GirlCategoryBean.class);
    }
