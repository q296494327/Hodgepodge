...
import pers.xiemiao.hodgepodge.bean.GirlDetailBean;
 ...
 * Desc: 美女分类网络协议
 */
public class GirlDetailProtocol {
     ...
     * @param params 数据加载类型
     */
    public GirlDetailBean loadData(String params) throws Exception {
        if (!isRefresh) {
            //1先去获取本地缓存
            GirlDetailBean localBean = getDataFromLocal(params);
        ...
        isRefresh = false;
        //2当本地缓存为空时，就去返回网络数据
        GirlDetailBean netBean = getDataFromNet(params);
        if (netBean != null) {
            return netBean;
        }
        return null;
     ...
     * @param params 数据加载类型
     */
    private GirlDetailBean getDataFromLocal(String params) {
        //1指定存储路径以及存储名字
     ...
     * @throws IOException
     */
    private GirlDetailBean getDataFromNet(String params) throws IOException {
        String url;
        if (params.substring(0, 1).equals("/")) {//如果参数的第一个数/,就请求列表图片
            url = Constants.URLS.GIRLDETAILURL + "&id=" + params;
        } else {
            url = Constants.URLS.GIRLCATEGORYURL + "&type=" + params;
        }
     ...
     */
    protected GirlDetailBean parseJson(String jsonString) {
        return new Gson().fromJson(jsonString, GirlDetailBean.class);
    }
