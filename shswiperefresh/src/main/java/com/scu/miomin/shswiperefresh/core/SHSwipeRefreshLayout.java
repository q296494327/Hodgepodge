...
public class SHSwipeRefreshLayout extends FrameLayout implements NestedScrollingParent {
    ...
    public SHSwipeRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr, int
            defStyleRes) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        if (resId == -1) {
            mGuidanceViewBgColor = ta.getColor(R.styleable
                    .SHSwipeRefreshLayout_guidance_view_bg_color,
                    Color.WHITE);
        } else {
        ...
        if (resId == -1) {
            mGuidanceViewTextColor = ta.getColor(R.styleable
                    .SHSwipeRefreshLayout_guidance_text_color,
                    Color.BLACK);
        } else {
        ...
        if (!canChildScrollUp() && mPullRefreshEnable && mCurrentAction == ACTION_PULL_REFRESH) {
            ...
            if (onRefreshListener != null) {
                if (lp.height >= guidanceViewHeight) {
                    onRefreshListener.onRefreshPulStateChange(lp.height / guidanceViewHeight,
                            OVER_TRIGGER_POINT);
                } else {
                    onRefreshListener.onRefreshPulStateChange(lp.height / guidanceViewHeight,
            ...
            if (onRefreshListener != null) {
                if (lp.height >= guidanceViewHeight) {
                    onRefreshListener.onLoadmorePullStateChange(lp.height / guidanceViewHeight,
                            OVER_TRIGGER_POINT);
                } else {
                    onRefreshListener.onLoadmorePullStateChange(lp.height / guidanceViewHeight,
                            NOT_OVER_TRIGGER_POINT);
